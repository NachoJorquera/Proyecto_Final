...
#def buscar_precio_minimo(listResult):
    precios_minimos = {}

    def actualizar_precio_minimo(acc, item):
        patron_busqueda = item['patron_busqueda']
        precio = item['precio']
        if patron_busqueda in acc:
            if precio < acc[patron_busqueda]:
                return {**acc, patron_busqueda: precio}
            else:
                return acc
        else:
            return {**acc, patron_busqueda: precio}
        
    precios_minimos = reduce(actualizar_precio_minimo, listResult, precios_minimos)
    return precios_minimos

#def mostrar_precios_minimo(precios_minimos):
    #def mostrar_precio_minimo(item):
        #patron_busqueda, precio_minimo = item
        #print('El precio mínimo para {} es {}'.format(patron_busqueda, precio_minimo))

    #list(map(mostrar_precio_minimo, precios_minimos.items()))
...
def main(L_FIND):
    ...
    #precios_minimos = buscar_precio_minimo(listResult)
   # mostrar_precio_minimo(precios_minimos)
